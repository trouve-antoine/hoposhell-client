...
pub fn make_size_message(master_pty: &Box<dyn portable_pty::MasterPty + Send>) -> Message<MessageTypeToStream>{
    ...
    payload.append(&mut "size".as_bytes().to_vec());
    payload.push(MESSAGE_PARTS_SEPARATOR);
    payload.append(&mut pty_size.rows.to_string().as_bytes().to_vec());
    payload.push(MESSAGE_PARTS_SEPARATOR);
    payload.append(&mut pty_size.cols.to_string().as_bytes().to_vec());
